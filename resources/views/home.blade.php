...
<head>
    ...
    /* Animasi Tema Geologi */
    @keyframes tectonic-shift {
        0% { transform: translateX(0) translateY(0) rotate(0deg); }
        50% { transform: translateX(20px) translateY(-10px) rotate(1deg); }
        100% { transform: translateX(0) translateY(0) rotate(0deg); }
    }
    .animate-tectonic {
        animation: tectonic-shift 20s ease-in-out infinite;
    }
    ...
    .animate-magma {
        animation: magma-flow 15s ease-in-out infinite;
    }
...
<body class="bg-[#f8fafc] text-slate-900 overflow-x-hidden">
    ...
    <!-- LAYANAN TERINTEGRASI SECTION -->
    <section class="relative py-32 px-6 overflow-hidden tech-gradient">
        <div class="absolute inset-0 pointer-events-none overflow-hidden">
            <div class="absolute inset-0 opacity-[0.07]" style="background-image: linear-gradient(to right, #ff6a1a 1px, transparent 1px), linear-gradient(to bottom, #ff6a1a 1px, transparent 1px); background-size: 80px 80px;"></div>
            <div class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[1200px] h-[1200px] rounded-full animate-[spin_25s_linear_infinite] opacity-30" style="background: conic-gradient(from 0deg, transparent 80%, rgba(255, 106, 26, 0.4) 100%);"></div>
        ...
        <div class="max-w-7xl mx-auto relative z-10">
            <div class="text-center mb-20 space-y-3">
                ...
                <h2 class="text-4xl md:text-5xl font-black text-slate-950 tracking-tighter uppercase leading-none">
                    Mengapa Memilih <span class="inline-block text-transparent bg-clip-text bg-gradient-to-r from-orange-600 to-amber-400 -skew-x-12">
    Kami ?
</span>
                </h2>
    ...
    <section class="py-24 bg-[#f8fafc] px-6 relative overflow-hidden border-t border-slate-100">
        ...
        <div class="max-w-7xl mx-auto relative z-10">
            ...
            <div class="grid grid-cols-1 md:grid-cols-12 gap-8">
                @if(isset($articleHighlight) && $articleHighlight)
                ...
                <div class="md:col-span-5 flex flex-col gap-5">
                    ...
                    <a href="{{ route('article.show', $art->slug) }}" class="group flex gap-5 bg-white p-5 rounded-[2.5rem] border border-slate-100 hover:border-[#C9490F] transition-all hover:shadow-xl shadow-sm">
                        <div class="w-28 h-28 rounded-3xl overflow-hidden flex-shrink-0 bg-slate-50 border border-slate-100">
                            <img src="{{ asset('storage/' . $art->thumbnail) }}" class="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700" alt="{{ $art->title }}">
                        </div>
    ...
    <section class="py-32 bg-slate-950 relative overflow-hidden border-t border-slate-800">
        <div class="absolute inset-0 pointer-events-none overflow-hidden">
            <div class="absolute inset-0 opacity-15" style="background-image: linear-gradient(to right, #FF6000 1.5px, transparent 1.5px), linear-gradient(to bottom, #FF6000 1.5px, transparent 1.5px); background-size: 70px 70px;"></div>
            ...
            <div class="text-center mb-20 space-y-4">
                ...
                <div class="space-y-4">
                    <h2 class="text-5xl md:text-6xl font-black text-white uppercase tracking-tighter leading-none">
                        Suara <span class="inline-block text-transparent bg-clip-text bg-gradient-to-r from-orange-600 to-amber-400 -skew-x-12">
    PELANGGAN
</span>
                    </h2>
    ...
    <!-- ========================================== -->
    <!-- MODAL PDF VIEWER UNTUK SERTIFIKAT -->
    <!-- ========================================== -->
    <div id="pdfModal" class="fixed inset-0 z-[200] hidden overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
        <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
            <!-- Overlay Gelap -->
            <div class="fixed inset-0 transition-opacity bg-slate-950/90 backdrop-blur-md" onclick="closePdfModal()"></div>

            <span class="hidden sm:inline-block sm:align-middle sm:h-screen" aria-hidden="true">&#8203;</span>
            
            <!-- Panel Modal -->
            <div class="inline-block align-bottom bg-slate-900 rounded-[2rem] text-left overflow-hidden shadow-2xl transform transition-all sm:my-8 sm:align-middle sm:max-w-4xl sm:w-full border border-slate-700">
                
                <!-- Header Modal -->
                <div class="bg-slate-800 px-6 py-4 flex justify-between items-center border-b border-slate-700">
                    <div class="flex items-center gap-3">
                        <i class="fa-solid fa-file-pdf text-[#C9490F] text-2xl"></i>
                        <div>
                            <h3 class="text-lg font-bold text-white uppercase tracking-widest">Dokumen Sertifikasi</h3>
                            <p class="text-slate-400 text-[10px] uppercase tracking-widest">K Survey Precision Technology</p>
                        </div>
                    </div>
                    <button onclick="closePdfModal()" class="text-slate-400 hover:text-white transition-colors bg-slate-700 hover:bg-slate-600 w-10 h-10 rounded-full flex items-center justify-center">
                        <i class="fa-solid fa-xmark text-xl"></i>
                    </button>
                </div>
                
                <!-- PDF Viewer Area -->
                <div class="w-full h-[75vh] bg-slate-950 relative">
                    <!-- Loading Indicator -->
                    <div id="pdfLoading" class="absolute inset-0 flex flex-col items-center justify-center z-0">
                        <i class="fa-solid fa-circle-notch fa-spin text-[#C9490F] text-4xl mb-3"></i>
                        <p class="text-slate-400 text-xs font-bold tracking-widest uppercase">Memuat Dokumen...</p>
                    </div>
                    <!-- Iframe Viewer -->
                    <iframe id="pdfIframe" src="" class="w-full h-full relative z-10 border-none rounded-b-none" onload="document.getElementById('pdfLoading').style.display='none'"></iframe>
                </div>
                
                <!-- Footer Modal -->
                <div class="bg-slate-800 px-6 py-4 flex justify-end border-t border-slate-700">
                    <a id="pdfDownloadBtn" href="" download class="px-6 py-3 rounded-xl bg-[#C9490F] text-white font-bold uppercase text-[10px] tracking-widest hover:bg-[#a63a0b] shadow-lg transition-all flex items-center gap-2">
    ...
    <!-- SCRIPTS -->
    <script>
        // JS untuk toggle Feedback Modal
        ...
        function openPdfModal(pdfUrl) {
            ...
            // Tampilkan loading & bersihkan src lama
            loading.style.display = 'flex';
            iframe.src = '';
            
            // Atur URL ke iframe dan tombol download
            // #toolbar=0&navpanes=0 membuat UI bawaan Chrome PDF viewer lebih bersih
        ...
        function closePdfModal() {
            ...
            // Kosongkan src iframe sedikit jeda agar tidak mengganggu transisi tutup
            setTimeout(() => {
                iframe.src = '';
            }, 300);
        }

        // JS untuk menutup modal menggunakan tombol ESC
        document.addEventListener('keydown', (e) => {
            if (e.key === "Escape") {
                const feedbackModal = document.getElementById('feedbackModal');
                const pdfModal = document.getElementById('pdfModal');
                
                if (!feedbackModal.classList.contains('hidden')) toggleModal('feedbackModal');
                if (!pdfModal.classList.contains('hidden')) closePdfModal();
            }
        });
    </script>
