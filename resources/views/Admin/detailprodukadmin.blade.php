...
<body class="text-slate-900 overflow-x-hidden transition-all duration-500 ease-[cubic-bezier(0.16,1,0.3,1)]"> 
    ...
    <header class="pt-32 pb-12 px-6 max-w-7xl mx-auto relative group">
        <div class="absolute top-20 right-6 z-50">
            <button onclick="openModal('detailModal')" class="bg-gradient-to-r from-orange-600 to-orange-500 text-white hover:brightness-110 transition-all duration-300 px-6 py-3 rounded-xl font-black text-[10px] uppercase tracking-widest flex items-center gap-2 shadow-lg shadow-orange-500/30 border border-white/10">
        ...
        <div class="flex flex-col md:flex-row md:items-end justify-between gap-6 border-b border-orange-100 pb-10">
            <div class="space-y-2">
                <span class="text-[#C9490F] font-bold text-xs uppercase tracking-widest">{{ $produk->brand }}</span>
    ...
    <main class="max-w-7xl mx-auto px-6 py-12">
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-16">
            ...
            <div class="lg:col-span-7 space-y-8 relative">
                <div class="bg-[radial-gradient(circle_at_center,#ffffff_0%,#fff7ed_100%)] aspect-[1.2/1] rounded-[3.5rem] flex items-center justify-center p-16 border border-orange-50 relative overflow-hidden group">
                    ...
                    <div class="absolute bottom-8 right-8 flex gap-3">
                         <button id="btnFullscreen" onclick="openFullscreen('{{ asset('storage/' . $produk->image) }}')" class="w-12 h-12 rounded-2xl bg-white/80 backdrop-blur shadow-sm border border-orange-100 flex items-center justify-center text-slate-400 hover:text-[#C9490F] transition-colors"><i class="fa-solid fa-expand"></i></button>
                    </div>
                </div>
            ...
            <div class="lg:col-span-5 space-y-12">
                <div class="bg-gradient-to-br from-[#170902] to-[#2a160b] shadow-[0_25px_50px_-12px_rgba(201,73,15,0.25)] p-10 rounded-[3rem] text-white relative overflow-hidden">
                    ...
                    <h3 class="text-xs font-black uppercase tracking-[0.3em] text-orange-400 mb-6">Deskripsi Produk</h3>
                ...
                <div class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 gap-4">
                        ...
                        @if($produk->advantages && count($produk->advantages) > 0)
                            ...
                            <div class="flex items-center gap-5 p-5 rounded-3xl bg-slate-50 border border-slate-100 hover:bg-white hover:border-orange-200 hover:shadow-xl hover:shadow-orange-500/10 transition-all duration-500 ease-[cubic-bezier(0.16,1,0.3,1)]">
        ...
        <div class="mt-32 space-y-12 relative border-t border-orange-100 pt-16">
            <div class="absolute top-16 right-0 z-50">
                <button onclick="openModal('specModal')" class="bg-gradient-to-r from-orange-600 to-orange-500 text-white hover:brightness-110 transition-all duration-300 px-6 py-3 rounded-xl font-black text-[10px] uppercase tracking-widest flex items-center gap-2 shadow-lg shadow-orange-500/30 border border-white/10">
                    <i class="fa-solid fa-list-check"></i> Kelola Spesifikasi Data
                </button>
            </div>

            <div class="flex flex-col gap-4">
                <p class="text-[11px] font-black text-[#C9490F] uppercase tracking-[0.4em]">Inside Technology</p>
                <h2 class="text-3xl md:text-4xl font-black text-slate-950 tracking-tight">Spesifikasi Teknis</h2>
            </div>
            
            <div class="bg-white rounded-[3.5rem] border-2 border-orange-100 p-8 md:p-12 shadow-sm shadow-orange-500/5 relative">
                ...
                <form action="{{ route('product.update', $produk->id) }}" method="POST" enctype="multipart/form-data" class="space-y-10">
                    ...
                    <div class="space-y-6 pt-4">
                        <div class="grid grid-cols-2 md:grid-cols-3 gap-6">
                            <div class="space-y-2">
                                <label class="text-[10px] font-black uppercase tracking-[0.2em] text-[#C9490F] ml-1">Sub-Judul (Kategori)</label>
                                <input type="text" name="brand" class="bg-[#F8FAFC] border border-[#E2E8F0] focus:bg-[#FFFFFF] focus:border-[#C9490F] outline-none w-full rounded-xl p-4 text-sm font-bold" value="{{ $produk->brand }}">
                            </div>
                            <div class="space-y-2">
                                <label class="text-[10px] font-black uppercase tracking-[0.2em] text-[#C9490F] ml-1">Badge (Label Atas)</label>
                                <input type="text" name="badge" class="bg-[#F8FAFC] border border-[#E2E8F0] focus:bg-[#FFFFFF] focus:border-[#C9490F] outline-none w-full rounded-xl p-4 text-sm font-bold" value="{{ $produk->badge }}">
                            </div>
                            <div class="space-y-2">
                                <label class="text-[10px] font-black uppercase tracking-[0.2em] text-[#C9490F] ml-1">Nama Produk Utama</label>
                                <input type="text" name="name" class="bg-[#F8FAFC] border border-[#E2E8F0] focus:bg-[#FFFFFF] focus:border-[#C9490F] outline-none w-full rounded-xl p-4 text-sm font-bold" value="{{ $produk->name }}">
                            </div>
                        </div>
                        <div class="space-y-2">
                            <label class="text-[10px] font-black uppercase tracking-[0.2em] text-[#C9490F] ml-1">Deskripsi Utama Detail (Halaman Detail)</label>
                            <textarea name="full_description" rows="4" class="bg-[#F8FAFC] border border-[#E2E8F0] focus:bg-[#FFFFFF] focus:border-[#C9490F] outline-none w-full rounded-xl p-4 text-sm font-medium resize-none leading-relaxed">{{ $produk->full_description }}</textarea>
                        </div>
                    </div>

                    <div class="space-y-2 pt-4 border-t border-slate-100">
                        <label class="text-[10px] font-black uppercase tracking-[0.2em] text-[#C9490F] ml-1">Upload Brosur Teknis</label>
                        <div class="flex items-center gap-4 bg-slate-50 p-4 rounded-2xl border border-slate-100">
                            <div class="w-12 h-12 bg-[#C9490F]/10 text-[#C9490F] border border-[#C9490F]/20 rounded-xl flex items-center justify-center text-xl shrink-0 shadow-sm"><i class="fa-solid fa-file-arrow-down"></i></div>
                            <div class="flex-grow">
                                @if($produk->brochure)
                                    <p class="text-sm font-bold text-slate-900">Brosur Tersedia</p>
                                    <p class="text-[10px] font-bold text-[#C9490F] uppercase tracking-widest mt-0.5">Siap di-download</p>
                                @else
                                    <p class="text-sm font-bold text-slate-400 italic">Belum ada brosur</p>
                                @endif
                            </div>
                            <div class="flex gap-2">
                                <label class="w-10 h-10 bg-white border border-slate-200 text-slate-500 rounded-xl hover:text-[#C9490F] hover:border-[#C9490F] transition-colors flex items-center justify-center cursor-pointer shadow-sm">
                    ...
                    <div class="pt-6 border-t border-slate-100">
                        <div class="flex justify-between items-center mb-5 border-b border-slate-100 pb-3">
                            <label class="text-[10px] font-black uppercase tracking-[0.2em] text-[#C9490F] ml-1"><i class="fa-solid fa-star mr-2"></i> Key Advantages (Keunggulan)</label>
                        </div>
                            ...
                            @if($produk->advantages && count($produk->advantages) > 0)
                                ...
                                <div class="flex gap-4 items-center bg-slate-50 p-4 rounded-2xl border border-slate-100">
                                    <div class="space-y-1 w-1/4">
                                        ...
                                        <input type="text" name="adv_icons[]" class="bg-[#F8FAFC] border border-[#E2E8F0] focus:bg-[#FFFFFF] focus:border-[#C9490F] outline-none w-full rounded-xl p-3 text-xs" value="{{ $adv['icon'] }}">
                                    </div>
                                    <div class="space-y-1 w-full">
                                        <label class="text-[8px] font-black uppercase text-slate-400">Judul Fitur</label>
                                        <input type="text" name="adv_titles[]" class="bg-[#F8FAFC] border border-[#E2E8F0] focus:bg-[#FFFFFF] focus:border-[#C9490F] outline-none w-full rounded-xl p-3 text-xs font-bold" value="{{ $adv['title'] }}">
                                    </div>
                                    <div class="space-y-1 w-full">
                                        <label class="text-[8px] font-black uppercase text-slate-400">Deskripsi Pendek</label>
                                        <input type="text" name="adv_descs[]" class="bg-[#F8FAFC] border border-[#E2E8F0] focus:bg-[#FFFFFF] focus:border-[#C9490F] outline-none w-full rounded-xl p-3 text-xs" value="{{ $adv['desc'] }}">
                                    </div>
                                    <div class="pt-4">
                    ...
                    <div class="bg-slate-50 px-10 py-6 border-t border-slate-200 flex justify-end gap-3 rounded-b-[2.5rem] -mx-8 -mb-8 mt-8">
                        ...
                        <button type="submit" class="bg-gradient-to-r from-orange-600 to-orange-500 text-white transition-all duration-300 hover:brightness-110 px-10 py-3.5 rounded-xl font-black text-[10px] uppercase tracking-widest shadow-lg shadow-orange-500/30 flex items-center gap-2"><i class="fa-solid fa-floppy-disk"></i> Simpan Perubahan Utama</button>
                    </div>
                </form>
            </div>
        </div>
    ...
    <div id="specModal" class="hidden fixed inset-0 z-[9999] bg-slate-900/85 backdrop-blur-sm items-center justify-center p-4 transition-all duration-300">
        ...
        <div class="relative bg-white w-full max-w-3xl rounded-[2.5rem] shadow-2xl flex flex-col max-h-[90vh] overflow-hidden border border-slate-200 mx-auto animate-in zoom-in-95 duration-300">
            <div class="bg-slate-50 px-8 py-6 border-b border-slate-200 flex justify-between items-center">
                <div class="flex items-center gap-4">
                    <div class="w-10 h-10 bg-gradient-to-br from-orange-600 to-orange-500 text-white rounded-xl flex items-center justify-center shadow-md"><i class="fa-solid fa-list-check"></i></div>
                    <div>
            ...
            <div class="p-8 overflow-y-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
                <form action="{{ route('product.update', $produk->id) }}" method="POST">
                    ...
                    <div class="space-y-3" id="specListAdmin">
                        @if($produk->specifications && count($produk->specifications) > 0)
                            ...
                            <div class="flex gap-4 items-center">
                                <input type="text" name="spec_labels[]" class="bg-[#F8FAFC] border border-[#E2E8F0] transition-all duration-200 focus:bg-[#FFFFFF] focus:border-[#C9490F] focus:outline-none w-1/3 rounded-xl p-3.5 text-xs font-bold text-slate-500 uppercase tracking-widest" value="{{ $spec['label'] }}">
                                <input type="text" name="spec_values[]" class="bg-[#F8FAFC] border border-[#E2E8F0] transition-all duration-200 focus:bg-[#FFFFFF] focus:border-[#C9490F] focus:outline-none w-full rounded-xl p-3.5 text-sm font-bold text-slate-900" value="{{ $spec['value'] }}">
                        ...
                        @else
                            <div class="flex gap-4 items-center">
                                <input type="text" name="spec_labels[]" class="bg-[#F8FAFC] border border-[#E2E8F0] focus:bg-[#FFFFFF] focus:border-[#C9490F] outline-none w-1/3 rounded-xl p-3.5 text-xs font-bold text-slate-500 uppercase tracking-widest" placeholder="Label Spesifikasi">
                                <input type="text" name="spec_values[]" class="bg-[#F8FAFC] border border-[#E2E8F0] focus:bg-[#FFFFFF] focus:border-[#C9490F] outline-none w-full rounded-xl p-3.5 text-sm font-bold text-slate-900" placeholder="Nilai Spesifikasi">
                                <button type="button" onclick="this.parentElement.remove()" class="w-12 h-12 bg-white border border-slate-200 text-slate-400 rounded-xl hover:bg-red-500 hover:text-white transition-colors flex items-center justify-center shrink-0"><i class="fa-solid fa-trash"></i></button>
                            </div>
                        @endif
                    </div>

                    <button type="button" onclick="addSpecRow()" class="mt-6 w-full py-4 border-2 border-dashed border-slate-300 rounded-xl text-[10px] font-black uppercase tracking-widest text-slate-400 hover:border-[#C9490F] hover:text-[#C9490F] transition-colors flex items-center justify-center gap-2 bg-slate-50">
                    ...
                    <div class="bg-slate-50 px-8 py-5 border-t border-slate-200 flex justify-end gap-3 rounded-b-[2.5rem] -mx-8 -mb-8 mt-8">
                        ...
                        <button type="submit" class="bg-gradient-to-r from-orange-600 to-orange-500 text-white transition-all duration-300 hover:brightness-110 px-8 py-3 rounded-xl font-black text-[10px] uppercase tracking-widest shadow-lg shadow-orange-500/30 flex items-center gap-2"><i class="fa-solid fa-check"></i> Simpan Spesifikasi</button>
                    </div>
                </form>
            </div>
        </div>
    ...
    <script>
        ...
        function addSpecRow() {
            ...
            row.innerHTML = `
                <input type="text" name="spec_labels[]" class="bg-[#F8FAFC] border border-[#E2E8F0] focus:bg-[#FFFFFF] focus:border-[#C9490F] outline-none w-1/3 rounded-xl p-3.5 text-xs font-bold text-slate-500 uppercase tracking-widest" placeholder="Label (ex: Berat)">
                <input type="text" name="spec_values[]" class="bg-[#F8FAFC] border border-[#E2E8F0] focus:bg-[#FFFFFF] focus:border-[#C9490F] outline-none w-full rounded-xl p-3.5 text-sm font-bold text-slate-900" placeholder="Nilai (ex: 1.5 Kg)">
                <button type="button" onclick="this.parentElement.remove()" class="w-12 h-12 bg-white border border-slate-200 text-slate-400 rounded-xl hover:bg-red-500 hover:text-white transition-colors flex items-center justify-center shrink-0"><i class="fa-solid fa-trash"></i></button>
            `;
        ...
        function addAdvRow() {
            ...
            row.innerHTML = `
                <div class="space-y-1 w-1/4">
                    ...
                    <input type="text" name="adv_icons[]" class="bg-[#F8FAFC] border border-[#E2E8F0] focus:bg-[#FFFFFF] focus:border-[#C9490F] outline-none w-full rounded-xl p-3 text-xs" placeholder="fa-star">
                </div>
                <div class="space-y-1 w-full">
                    <label class="text-[8px] font-black uppercase text-slate-400">Judul Fitur</label>
                    <input type="text" name="adv_titles[]" class="bg-[#F8FAFC] border border-[#E2E8F0] focus:bg-[#FFFFFF] focus:border-[#C9490F] outline-none w-full rounded-xl p-3 text-xs font-bold" placeholder="Judul Keunggulan">
                </div>
                <div class="space-y-1 w-full">
                    <label class="text-[8px] font-black uppercase text-slate-400">Deskripsi Pendek</label>
